...
-- We define the rotate and its inverse then use both to prove bijection
mutual
def rotate (T : full_bin_tree) : plane_tree := plane_tree.join (rotate_list T)

def rotate_list : full_bin_tree -> List plane_tree
  ...
  | full_bin_tree.join T1 T2 => (rotate T1) :: (rotate_list T2)
end

mutual
def rotate_inv : plane_tree -> full_bin_tree
  | plane_tree.join [] => full_bin_tree.leaf
  | plane_tree.join (T1 :: L) => full_bin_tree.join (rotate_inv T1) (rotate_inv_list L)

def rotate_inv_list : List plane_tree -> full_bin_tree
  | [] => full_bin_tree.leaf
  | T1 :: L => full_bin_tree.join (rotate_inv T1) (rotate_inv_list L)
end
...
def size_eq : T.size + 1 = (rotate T).size ∧ T.size + 1 = plane_tree.size (plane_tree.join (rotate_list T)) := by
  match T with
  | full_bin_tree.leaf => rw [rotate, full_bin_tree.size, rotate_list, plane_tree.size]
                          simp
  | full_bin_tree.join T1 T2 => rw [rotate, rotate_list, plane_tree.size]
                                have t1 : full_bin_tree.size T1 + 1 = plane_tree.size (rotate T1) := And.left size_eq
                                have t2 : T2.size + 1 = plane_tree.size (plane_tree.join (rotate_list T2)) := And.right size_eq
                                rw [<- t1, full_bin_tree.size, <- t2]
                                simp
                                rw [Nat.add_comm, <- Nat.add_assoc, Nat.add_comm, Nat.add_comm (full_bin_tree.size T1)]
                                have t3 : full_bin_tree.size T2 + 1 = plane_tree.size (rotate T2) := And.left size_eq
...
-- We show (rotate_inv (rotate T) = T) with structural induction
theorem inv_rot_ind (T : full_bin_tree) : rotate_inv (rotate T) = T ∧ rotate_inv_list (rotate_list T) = T := by
  rw [rotate]
  apply And.intro
  case left =>
    match T with
    | full_bin_tree.leaf => rw [rotate_list, rotate_inv]
    | full_bin_tree.join T1 T2 => rw [rotate_list, rotate_inv]
                                  have t : (rotate_inv (rotate T1)) = T1 := And.left (inv_rot_ind T1)
                                  rw [t]
                                  match T2 with
                                  | full_bin_tree.leaf => rw [rotate_list, rotate_inv_list]
                                  | full_bin_tree.join T11 T12 => rw [rotate_list, rotate_inv_list]
                                                                  have t1 : rotate_inv (rotate T11) = T11 := And.left (inv_rot_ind T11)
                                                                  have t2 : rotate_inv_list (rotate_list T12) = T12 :=  And.right (inv_rot_ind T12)
                                                                  rw [t1, t2]
  case right =>
    match T with
    | full_bin_tree.leaf => rw [rotate_list, rotate_inv_list]
    | full_bin_tree.join T1 T2 => rw [rotate_list, rotate_inv_list]
                                  have t : (rotate_inv (rotate T1)) = T1 := And.left (inv_rot_ind T1)
                                  rw [t]
                                  have t2 : rotate_inv_list (rotate_list T2) = T2 :=  And.right (inv_rot_ind T2)
                                  rw [t2]

theorem inv_rot (T : full_bin_tree) : rotate_inv (rotate T) = T := And.left (inv_rot_ind T)

theorem inv_sur (T : full_bin_tree) : T = full_bin_tree.leaf ∨ ∃ R, ∃ L, T = rotate_inv (plane_tree.join (R :: L)) := by
match T with
  | full_bin_tree.leaf => simp
  | full_bin_tree.join T1 T2 => simp
                                have r : (rotate_inv_list (rotate_list T2)) = T2 := (And.right (inv_rot_ind T2))
                                have t : rotate_inv (rotate T1) = T1 := (And.left (inv_rot_ind T1))
                                have x : rotate_inv (plane_tree.join ((rotate T1) :: (rotate_list T2))) = full_bin_tree.join T1 T2 := by
                                  rw [rotate_inv, t, r]
                                rw [<- x]
                                apply Exists.intro
                                apply Exists.intro
...
-- Wasn't used
theorem rot_inv_list_ind : (∀ (R : plane_tree), R ∈ L1::L2 -> rotate (rotate_inv R) = R) -> (∀ (R : plane_tree), R ∈ L2 -> rotate (rotate_inv R) = R) := by
  intro h
  simp at h
  exact And.right h

-- Wasn't used
theorem rot_inv_implies_rot_inv_list : (∀ (R : plane_tree), R ∈ L -> rotate (rotate_inv R) = R) -> rotate_list (rotate_inv_list L) = L := by
  intro h
  match L with
  | [] => rw [rotate_inv_list, rotate_list]
  | L1 :: L2 => have t : L1 ∈ L1 :: L2 := by simp
                have r : rotate (rotate_inv L1) = L1 := (h L1) t
                rw [rotate_inv_list, rotate_list, r]
                simp at h
                have y : ∀ (R : plane_tree), R ∈ L2 → rotate (rotate_inv R) = R := And.right h
                have z : rotate_list (rotate_inv_list L2) = L2 := rot_inv_implies_rot_inv_list y
                rw [z]
...
-- We show (rotate (rotate_inv T) = T) with induction on number of leaves
theorem rot_inv_ind (n : Nat) : ∀ ⦃R L⦄, (plane_tree.join (R :: L)).leaf_num ≤ n + 1 -> rotate (rotate_inv (plane_tree.join (R :: L))) = (plane_tree.join (R :: L)) ∧ rotate (rotate_inv R) = R ∧ (rotate_list (rotate_inv_list L) = L) := by
  induction n with
  | zero => intro R L
            intro h
            simp at h
  ...
  | succ k h => intro T1 T2
                intro r
                match T1 with
                | plane_tree.join [] => rw [rotate_inv, rotate, rotate_inv, rotate_list, rotate_inv, rotate, rotate_list]
                                        simp
                                        match T2 with
                                        | [] => rw [rotate_inv_list, rotate_list]
                                        | R1 :: L1 => rw [rotate_inv_list, rotate_list]
                                                      ...
                                                      have x : rotate (rotate_inv (plane_tree.join (R1 :: L1))) = plane_tree.join (R1 :: L1) ∧ rotate (rotate_inv R1) = R1 ∧  rotate_list (rotate_inv_list L1) = L1 := h t
                                                      rw [(And.left (And.right x))]
                                                      rw [(And.right (And.right x))]
                | plane_tree.join (R :: L) => rw [rotate_inv, rotate, rotate_list]
                                              ...
                                              have t : plane_tree.leaf_num (plane_tree.join (R :: L)) ≤ k + 1 := by
                                                ...
                                                exact t1
                                              have x : rotate (rotate_inv (plane_tree.join (R :: L))) = plane_tree.join (R :: L) := And.left (h t)
                                              rw [x]
                                              simp
                                              have t : plane_tree.leaf_num (plane_tree.join (plane_tree.join [] :: T2)) ≤ succ k := by
                                                ...
                                                exact w
                                              have x : rotate_list (rotate_inv_list T2) = T2 := And.right (And.right (h t))
                                              exact x

theorem rot_inv (T : plane_tree) : (rotate (rotate_inv T)) = T := by
  match T with
  | plane_tree.join [] => rw [rotate_inv, rotate, rotate_list]
  | plane_tree.join (R :: L) => have x : plane_tree.leaf_num (plane_tree.join (R :: L)) ≤ plane_tree.leaf_num (plane_tree.join (R :: L)) + 1 := Nat.le_add_right (plane_tree.leaf_num (plane_tree.join (R :: L))) 1
                                have t : rotate (rotate_inv (plane_tree.join (R :: L))) = (plane_tree.join (R :: L)) ∧ rotate (rotate_inv R) = R ∧ (rotate_list (rotate_inv_list L) = L) := rot_inv_ind ((plane_tree.join (R :: L)).leaf_num) x
                                exact And.left t

theorem rot_inv_list (L : List plane_tree) : rotate_list (rotate_inv_list L) = L := by
  ...
  have t : rotate (rotate_inv (plane_tree.join (plane_tree.join [] :: L))) = (plane_tree.join (plane_tree.join [] :: L)) ∧ rotate (rotate_inv (plane_tree.join [])) = plane_tree.join [] ∧ (rotate_list (rotate_inv_list L) = L) := rot_inv_ind ((plane_tree.join (plane_tree.join [] :: L)).leaf_num) x
  exact And.right (And.right t)

theorem inv_rot_list (T : full_bin_tree) : rotate_inv_list (rotate_list T) = T := And.right (inv_rot_ind T)

theorem rotate_injective : injective rotate := by
  unfold injective
  intro x y
  intro h
  match x, y with
  | full_bin_tree.leaf, full_bin_tree.leaf => simp
  | full_bin_tree.leaf, full_bin_tree.join T1 T2 => rw [rotate, rotate_list, rotate, rotate_list] at h
                                                    simp at h
  | full_bin_tree.join T1 T2, full_bin_tree.leaf => rw [rotate, rotate_list, rotate, rotate, rotate_list] at h
                                                    simp at h
  | full_bin_tree.join L1 L2, full_bin_tree.join R1 R2 => have t : ∃ R, ∃ L, full_bin_tree.join L1 L2 = rotate_inv (plane_tree.join (R :: L)) := by
                                                            have t : full_bin_tree.join L1 L2 = full_bin_tree.leaf ∨ ∃ R, ∃ L, full_bin_tree.join L1 L2 = rotate_inv (plane_tree.join (R :: L)) := inv_sur (full_bin_tree.join L1 L2)
                                                            simp at t
                                                            exact t
                                                          match t with
                                                          | ⟨R, L, t⟩ => rw [t] at h
...
theorem rotate_surjective : surjective rotate := by
  unfold surjective
  intro y
  match y with
  | plane_tree.join [] => have t : rotate (full_bin_tree.leaf) = plane_tree.join [] := by
                            rw [rotate, rotate_list]
                          apply Exists.intro
                          trivial
  | plane_tree.join (R :: L) => have t : rotate (rotate_inv (plane_tree.join (R :: L))) = plane_tree.join (R :: L) := rot_inv (plane_tree.join (R :: L))
                                apply Exists.intro
                                trivial

theorem rotate_bijective : bijective rotate := by
  unfold bijective
  exact And.intro rotate_injective rotate_surjective
...
theorem le_fac (n : Nat) : n ≤ fac n := by
  ...
  | zero => rw [zero_eq, fac]
            exact Nat.zero_le 1
  | succ m => rw [Nat.succ_eq_add_one, fac]
              have r : m + 1 ≤ m + 1 := by
